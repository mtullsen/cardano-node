...
runQueryStakePools
  ...
  -> Maybe (File () Out)
  -> ExceptT ShelleyQueryCmdError IO ()
runQueryStakePools mNodeSocketPath (AnyConsensusModeParams cModeParams) network mOutFile = runOopsInExceptT @ShelleyQueryCmdError $ do
  -- SocketPath sockPath <- maybe (lift readEnvSocketPath) (pure . Right) mNodeSocketPath
  --   & onLeft (left . ShelleyQueryCmdEnvVarSocketErr)

  SocketPath sockPath <- maybe (liftIO readEnvSocketPath) (pure . Right) mNodeSocketPath
