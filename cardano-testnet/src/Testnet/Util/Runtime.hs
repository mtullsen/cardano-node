...
startNode tempBaseAbsPath tempAbsPath logDir socketDir node nodeCmd = do
  ...
  createProcessNode <- H.procNode (nodeCmd <>
  ...
  -- TODO: We now fail early but the error should be appended to the end of
  -- stdout
  --errs <- liftIO $ IO.hGetContents stdErr
--
  --unless (null errs) $ do
  --  H.onFailure . H.note_ $ "Node " <> node <> " stderr: " <> errs
